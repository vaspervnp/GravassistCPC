...
εργαλείο είναι ότι **αλλάζει την κατεύθυνση της βαρύτητας**, πάντα ανά 90 μοίρες.
...
Ο ήρωας ζωγραφίζεται **μόνο σε μία φορά βαρύτητας** — τις άλλες τρεις τις παράγει
ο κώδικας. Λεπτομέρειες: [docs/sprites.md](docs/sprites.md).
...
**Τα sprites δεν αποθηκεύονται σε μορφή οθόνης.** Κρατιούνται ως 1 byte ανά pixel, σε
μία μόνο φορά βαρύτητας, και μια ρουτίνα κάνει σε μία πέραση περιστροφή + packing σε
MODE 1 + μετατόπιση x. Κοστίζει ~6% ενός frame και κερδίζει 3.5x μνήμη, κίνηση
ακριβείας 1 pixel, και το ένα τέταρτο της δουλειάς σχεδίασης — ο ίδιος κώδικας
εξυπηρετεί ήρωα και αντικείμενα. Δες [docs/sprites.md §1](docs/sprites.md).
