...
Ο editor μπορεί να ζητά **σύνδεση με Google**. Ενεργοποιείται μόνο όταν υπάρχουν και
οι δύο μεταβλητές περιβάλλοντος — **ποτέ σε αρχείο του repo**:
...
Χωρίς τις μεταβλητές ο editor τρέχει **όπως πάντα**, χωρίς σύνδεση: είναι εργαλείο
καθημερινής χρήσης και το να μην ανοίγει επειδή λείπει μια μεταβλητή θα ήταν χειρότερο
από το να μην έχει λογαριασμούς. Με τις μεταβλητές, **κάθε** σελίδα και κάθε API θέλει
λογαριασμό.

> **ΠΡΟΣΟΧΗ:** δεν υπάρχει λίστα επιτρεπόμενων. Με τη σύνδεση ενεργή, **οποιοσδήποτε**
> λογαριασμός Google μπορεί να μπει και να γράψει στα `levels/`. Πριν το ανεβάσεις
> κάπου δημόσια, χρειάζεται περιορισμός σε συγκεκριμένα email.
