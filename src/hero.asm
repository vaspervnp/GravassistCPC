...
hu_td:          ld   (h_td),a
                ...
                ; ΦΥΛΑΞΕ ΤΗΝ ΠΑΛΙΑ ΘΕΣΗ — ΜΗΝ ΣΒΗΝΕΙΣ ΑΚΟΜΑ. Το σβήσιμο θέλει
                ; αυτήν εδώ τη θέση, αλλά πρέπει να γίνει ΚΟΛΛΗΤΑ στο νέο σχέδιο:
                ; όσο γινόταν εδώ, το βέλος έλειπε από την οθόνη όσο έτρεχε η
                ; υπόλοιπη φυσική, το prep_hero και η αναμονή του flyback —
                ; δηλαδή τρεμόπαιζε. Σβήσιμο και σχεδίαση μαζί, στο main.asm,
...
; Η σημαία «διπλής κατεύθυνσης» της πόρτας ΔΕΝ διαβάζεται πια εδώ. Κρίνεται
; στην πόρτα από την οποία ΒΓΑΙΝΕΙΣ και έχει ήδη ληφθεί υπόψη όταν χτίστηκε ο
; πίνακας room_N_arr: αν λείπει εγγραφή, δεν υπάρχει σημείο άφιξης. Έμενε εδώ
; ως νεκρή αποθήκευση, που είναι χειρότερη από καθόλου — δείχνει ότι κάποιος
; τη διαβάζει.
...
rl_have:        pop  af
                ...
                ; Ο ήρωας εμφανίζεται στο σημείο άφιξης της πόρτας που γυρίζει
                ; πίσω. Ο πίνακας room_N_arr έχει εγγραφή ΜΟΝΟ για τις πόρτες
                ; που το δικαιούνται, οπότε εδώ δεν κρίνεται τίποτα: αν λείπει
                ; η εγγραφή, μένει το σημείο εκκίνησης της αίθουσας.
                ;
                ; Παλιά η απόφαση κρινόταν από τη σημαία της πόρτας που ΜΠΗΚΕΣ,
                ; δηλαδή από ΑΛΛΟ αρχείο: το σημείο άφιξης που είχες ορίσει
                ; αγνοούνταν επειδή έλειπε μια σημαία σε άλλη αίθουσα.
                call rl_arrival

                ; Καθαρή αρχή ΜΟΝΟ για ό,τι ανήκει στην αίθουσα. Η ενέργεια,
                ; τα κλειδιά, τα αλεξίπτωτα και το κιβώτιο που κουβαλάς είναι
                ; του ΠΑΙΚΤΗ και ταξιδεύουν μαζί του — το hero_carry μηδενιζόταν
                ; εδώ και το κιβώτιο εξαφανιζόταν περνώντας την πόρτα.
                xor  a
                ld   (crates_on),a      ; τα κιβώτια ξεκινούν ακίνητα
                ld   (hero_paraopen),a  ; το αλεξίπτωτο κλείνει στην προσγείωση
...
hu_islock:
                ...
                ld   a,(h_cell)         ; …στην ανοιχτή μορφή ΤΟΥ ΤΥΠΟΥ που
                ld   e,1                ; πάτησες — καρφωμένο T_LOCK_OPEN θα
                push hl                 ; μεταμόρφωνε την πύλη σε λουκέτο
                call tgt_want
                pop  hl
                call cell_set
...
tp_arrow:       call ta_erase           ; ό,τι φαινόταν πριν, φεύγει πρώτο
                ...
                ; ΔΙΠΛΑ ΤΟΥ ΚΑΙ ΣΕ ΚΕΝΟ ΚΕΛΙ. Πάνω του θα έκρυβε το πλακίδιο
                ; που δείχνει· σε γεμάτο κελί θα έκρυβε κάτι άλλο. Η αναζήτηση
                ; ξεκινά από τη μεριά ΑΠ' ΟΠΟΥ ΕΡΧΕΣΑΙ — εκεί το βελάκι πέφτει
                ; ανάμεσα στις δύο πλατφόρμες και διαβάζεται σαν διαδρομή.
                add  a,4                ; A = η φορά· ξεκινάμε αντίθετά της
                and  7
                ld   (ta_k),a
                ld   b,8
ta_try:         push bc
                ...
                ld   hl,gstep
                add  hl,de
                ld   a,(ta_cell)        ; στήλη προορισμού + βήμα
                add  a,(hl)             ; τα -1 είναι 255: το 8-bit add και το
                cp   LVL_COLS           ; cp κόβουν σωστά και τις δύο άκρες
                jr   nc,ta_skip
                ld   c,a
                inc  hl
                ld   a,(ta_cell+1)      ; γραμμή προορισμού + βήμα
                add  a,(hl)
                cp   LVL_ROWS
                jr   nc,ta_skip
                ld   b,a
                push bc
                call cell_rc            ; A = ο τύπος του (B=γραμμή, C=στήλη)
                pop  bc
                or   a
                jr   nz,ta_skip         ; όχι κενό: δοκίμασε τον επόμενο
                ld   (ta_cell),bc       ; βρέθηκε: εκεί μπαίνει το βελάκι
                ld   a,(ta_k)           ; …και δείχνει ΠΡΟΣ τον προορισμό
                add  a,4
                and  7
                ld   (ta_dir),a
                pop  bc
...
ta_skip:        ld   hl,ta_k
                ...
                pop  bc
                djnz ta_try
                ; Τίποτα ελεύθερο γύρω του: πάνω του. Ένα σημάδι που κρύβει
                ; κάτι είναι καλύτερο από κανένα σημάδι.
ta_draw:        ld   bc,(ta_cell)       ; C = στήλη, B = γραμμή
