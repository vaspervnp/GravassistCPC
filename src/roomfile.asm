...
; Το AMSDOS θέλει buffer 2 KB για να διαβάσει. Δανειζόταν τα πρώτα 2 KB της
; ΟΘΟΝΗΣ, γιατί δεν περίσσευαν αλλού — και αυτό ρίγωνε την οθόνη υποδοχής σε
; κάθε διάβασμα, αφού τα #C000..#C7FF είναι τα πρώτα 80 bytes καθεμιάς από τις
; οκτώ ενδιάμεσες ζώνες, δηλαδή μια λωρίδα κάθε όγδοη γραμμή σε ΟΛΗ την οθόνη.
;
; Αφότου κόπηκαν τα δεκαοκτώ αχρησιμοποίητα καρέ του ήρωα, τα 2 KB υπάρχουν.
; Ο buffer είναι δικός μας (cas_buffer, στο τέλος του main.asm) και η εικόνα
; υποδοχής μένει καθαρή όσο γεμίζουν οι τράπεζες.

;---------------------------------------------------------------------
; rle_unpack — ξεδιπλώνει το RLE σε LVL_CELLS bytes
;
;   ΕΝΑ byte ανά μεμονωμένο κελί, δύο ανά σειρά: το bit 7 του τύπου λέει «το
;   πλήθος ακολουθεί». Οι μισές διαδρομές των πιστών είναι μεμονωμένα κελιά
;   και τα σκέτα ζεύγη πλήρωναν 2 bytes για το καθένα — το πλέγμα των έξι
;   αιθουσών έπεσε από 1276 σε 956 bytes με αυτή τη σημαία και μόνο.
;
...
ru_pair:        ld   a,h
                or   l
                ret  z
                ld   a,(ix+0)           ; τύπος, με τη σημαία στο bit 7
                inc  ix
                ld   c,1                ; χωρίς σημαία: ένα και μόνο κελί
                bit  7,a
                jr   z,ru_have
                and  #7F                ; βγάλε τη σημαία από τον τύπο…
                ld   c,(ix+0)           ; …και διάβασε το πλήθος της σειράς
                inc  ix
ru_have:        ld   b,0
...
;---------------------------------------------------------------------
; set_load — φέρνει το σετ A στον set_buf, από τράπεζα αν υπάρχει
;
;   Ο ΜΟΝΟΣ δρόμος προς τον set_buf. Ό,τι ακολουθεί — room_find, rle_unpack,
;   οι τέσσερις δείκτες πινάκων, όλες οι σαρώσεις του παιχνιδιού — δεν ξέρει
;   ούτε χρειάζεται να ξέρει από πού ήρθαν τα bytes.
;
;   Η τράπεζα είναι ΚΡΥΦΗ ΜΝΗΜΗ, όχι νέα μορφή αρχείου: τις θέσεις τις γεμίζει
;   στην εκκίνηση το bank_boot από τα ίδια ROOMSnn.BIN. Χωρίς 128 KB, χωρίς
;   γεμάτη θέση, ή με στοίβα σε λάθος μέρος, ο δρόμος είναι η δισκέτα —
;   αμετάβλητος, όπως δούλευε πάντα.
;
; IN:  A = δείκτης σετ (1..)
; OUT: CF=1 επιτυχία· CF=0 αποτυχία (το σετ μένει ό,τι ήταν)
; ΑΛΛΟΙΩΝΕΙ: τα πάντα
;---------------------------------------------------------------------
set_load:       ld   (sl_idx),a
                ld   a,(bank_ok)
                or   a
                jr   z,sl_disc          ; μηχάνημα 64K
                ld   a,(sl_idx)
                call slot_full
                jr   nc,sl_disc         ; η θέση του δεν γέμισε ποτέ
                ld   a,(sl_idx)
                call slot_copy          ; 6 ms αντί για μισό δευτερόλεπτο
                call set_check          ; ΙΔΙΟΣ έλεγχος: πιάνει και κακή θέση
                jr   nc,sl_disc         ; χαλασμένη τράπεζα -> ξαναδοκίμασε δίσκο
                ld   a,(sl_idx)
                ld   (set_cur),a
                scf
                ret
...
sl_disc:        ld   a,(sl_idx)
                ; πέφτει μέσα στο set_disc

;---------------------------------------------------------------------
; set_disc — φορτώνει το ROOMSnn.BIN με δείκτη A (1..99) στον set_buf
;
; IN:  A = δείκτης σετ
; OUT: CF=1 επιτυχία· CF=0 αποτυχία (το σετ μένει ό,τι ήταν)
; ΑΛΛΟΙΩΝΕΙ: τα πάντα
...
;---------------------------------------------------------------------
; set_check — είναι ο set_buf πραγματικό σετ αιθουσών;
;
;   Καλείται από ΚΑΙ ΤΟΥΣ ΔΥΟ δρόμους — δισκέτα και τράπεζα. Στον δεύτερο
;   δεν είναι τελετουργικό: αν η αρίθμηση των θέσεων ξεφύγει, ή αν η τράπεζα
;   δεν κράτησε, εδώ φαίνεται — και ο set_load ξαναπέφτει στη δισκέτα αντί
;   να ξεδιπλώσει σκουπίδια σε αίθουσα.
;
; OUT: CF=1 εντάξει
; ΑΛΛΟΙΩΝΕΙ: AF, HL
;---------------------------------------------------------------------
;   ΠΡΟΣΟΧΗ ΣΤΟ CARRY: το `cp` το αφήνει σηκωμένο όταν το A είναι μικρότερο
;   από τον συγκρινόμενο, οπότε ένα σκέτο `ret nz` θα γύριζε «επιτυχία» σε
;   μισά από τα χαλασμένα αρχεία. Η αποτυχία περνά ρητά από το sc_no.
set_check:      ld   hl,set_buf         ; υπογραφή 'GRS': ένα λάθος αρχείο θα
                ...
                ; ΚΑΙ Η ΕΚΔΟΣΗ. Η υπογραφή μόνη της δεν φτάνει: όταν άλλαξε η
                ; κωδικοποίηση του πλέγματος, μια παλιά δισκέτα περνούσε το
                ; 'GRS' και ξεδιπλωνόταν σε σκουπίδια-αίθουσες — σφάλμα που
                ; μοιάζει με κατεστραμμένη πίστα, όχι με λάθος αρχείο.
                inc  hl
                ld   a,(hl)
                cp   SET_VERSION
                jr   nz,sc_no
                scf
                ret
sc_no:          or   a                  ; CF=0 ΡΗΤΑ, όχι ό,τι άφησε το cp
                ret
...
;---------------------------------------------------------------------
; skip_targ — ο πέμπτος πίνακας: ΤΕΤΡΑΔΕΣ (στήλη, γραμμή, φόρτιση, ρυθμός)
; IN/OUT: HL      ΑΛΛΟΙΩΝΕΙ: AF, HL
;---------------------------------------------------------------------
skip_targ:      ld   a,(hl)
                inc  hl
...
;---------------------------------------------------------------------
; skip_plat — ο έκτος πίνακας: ΟΚΤΑΔΕΣ
;   (στήλη, γραμμή, πλάτος, ύψος, dστήλη, dγραμμή, κανάλι, ταχύτητα)
;
; ΤΟ ΜΕΓΕΘΟΣ ΤΑΞΙΔΕΥΕΙ ΜΑΖΙ: τα κελιά «M» σβήνονται από το πλέγμα πριν γραφτεί
; το RLE — δείχνουν πού ξεκινάει, δεν είναι υλικό — οπότε ο Z80 δεν έχει από
; πού αλλού να μάθει πόσο μεγάλη είναι.
; IN/OUT: HL      ΑΛΛΟΙΩΝΕΙ: AF, BC, HL
;---------------------------------------------------------------------
skip_plat:      ld   a,(hl)
                inc  hl
                cp   #FF
...
;---------------------------------------------------------------------
; turret_arg — οι δύο χρόνοι του πυργίσκου (B,C) = (col,row)
;   OUT: D = φόρτιση, E = ρυθμός, σε δευτερόλεπτα· 0,0 αν δεν δηλώθηκε
;   ΑΛΛΟΙΩΝΕΙ: AF, DE, HL
;---------------------------------------------------------------------
turret_arg:     ld   hl,(room_targ)
ta_lp:          ld   a,(hl)
                cp   #FF
                jr   z,ta_none
                cp   b
...
gate_toggle:    or   a
                ...
                ; σταματούσε μόνο όσο κάθεται στην αφετηρία της.
                push af
                call plat_toggle
                pop  af
                ld   a,(gt_chan)
...
tgt_tab:        db   T_GATE,    T_GATE_OPEN
                ...
                ; Ο πυργίσκος: «ανοιχτό» σημαίνει ΑΚΙΝΔΥΝΟ, όπως και στα
                ; αγκάθια — ο διακόπτης τον σβήνει, δεν τον ανάβει.
                db   T_TURRET_V, T_TURRET_V_OFF
                db   T_TURRET_H, T_TURRET_H_OFF
...
room_targ       dw  0                   ; οι δύο χρόνοι κάθε πυργίσκου
room_plat       dw  0                   ; οι κινούμενες πλατφόρμες
