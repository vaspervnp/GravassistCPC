...
if DEMO_MODE
                ...
                ; ΤΟ ml_dir ΜΕΝΕΙ ΑΠΟ ΤΗΝ ΠΡΟΗΓΟΥΜΕΝΗ ΠΑΡΤΙΔΑ, και το anim_frame
                ; παίζει βήμα όποτε δεν είναι μηδέν. Στην πρώτη εκκίνηση είναι
                ; μηδέν και η αρένα ήταν πάντα βουβή· μετά από παρτίδα όμως ο
                ; ήρωας της επίδειξης άρχιζε να πατάει — στο κανάλι B, δηλαδή
                ; πάνω στο lead, και κάθε βήμα το άφηνε λίγο πιο πίσω από το
                ; μπάσο και τα τύμπανα. Το tools/test_music.py το φυλάει.
                xor  a
                ld   (ml_dir),a
                ; Και το M: αν το κρατούσε πατημένο φεύγοντας από την παρτίδα
                ; (εκεί είναι το βάδισμα μπροστά), η πρώτη περιστροφή του
                ; βρόχου θα το έβλεπε ως καινούργιο πάτημα και θα έσβηνε τη
                ; μουσική μόνη της. Ξεκινάμε θεωρώντας το πατημένο.
                ld   a,1
                ld   (menu_mheld),a
