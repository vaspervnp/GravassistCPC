...
;=====================================================================
                ; Η ΟΘΟΝΗ ΥΠΟΔΟΧΗΣ ΜΕΝΕΙ ΟΡΘΙΑ ΟΣΟ ΦΟΡΤΩΝΟΥΜΕ. Το SCR_SET_MODE
                ; σβήνει την οθόνη, οπότε αν μπει πρώτο ο παίκτης κοιτάζει μαύρο
                ; για όσο διαρκεί το γέμισμα των τραπεζών. Μπαίνει ΜΕΤΑ, όταν
                ; υπάρχει πια μενού να δείξουμε.
...
; ΤΟ ΣΕΤ ΑΙΘΟΥΣΩΝ ΔΕΝ ΖΕΙ ΣΤΗΝ ΚΥΡΙΑ ΜΝΗΜΗ — ζει στην ΟΘΟΝΗ.
;
; Κρατούσε ολόκληρο το σετ όσο έπαιζες, ενώ το παιχνίδι διαβάζει μόνο τους
; τέσσερις μικρούς πίνακες της τρέχουσας αίθουσας. Έτρωγε την τελευταία RAM
; και κάθε νέο feature το έσπρωχνε κάτω από το μέγεθος μιας αίθουσας.
;
; Τώρα φορτώνεται στα δεύτερα 2 KB της οθόνης, ΖΕΙ ΜΟΝΟ ΟΣΟ ΔΙΑΡΚΕΙ Η
; ΦΟΡΤΩΣΗ, και το πρώτο render το σβήνει. Ό,τι χρειάζεται μετά — οι τέσσερις
; πίνακες — αντιγράφεται στο tab_buf. Τα πρώτα 2 KB τα κρατά ήδη το AMSDOS
; για δικό του buffer, οπότε τα δύο δεν πατάνε το ένα το άλλο.
;
; ΣΥΝΕΠΕΙΑ: το set_cur δεν μπορεί πια να λειτουργεί ως κρυφή μνήμη — το
; περιεχόμενο του buffer καταστρέφεται από την πρώτη σχεδίαση. Το room_load
; ξαναφορτώνει πάντα· με τις τράπεζες αυτό είναι ένα LDIR.
set_buf         equ  CAS_BUFFER+2048
set_capacity    equ  2048
                assert set_buf+set_capacity <= #10000
                ; Οι τέσσερις πίνακες της ΤΡΕΧΟΥΣΑΣ αίθουσας, αντιγραμμένοι
                ; έξω από την οθόνη. Το tools/roomfile.py ελέγχει ότι καμία
                ; αίθουσα δεν τους ξεπερνά.
tab_buf         ds   TAB_BYTES
