...
SPR_MAXW        equ 5           ; μέγιστο πλάτος εξόδου (αλεξίπτωτο 16px + 3 shift)
...
;---------------------------------------------------------------------
; spr_save_para — αντιγράφει το αποτέλεσμα του spr_transform στο para_buf
;   Το spr_transform έχει ΕΝΑ buffer· το αλεξίπτωτο μετασχηματίζεται πρώτο
;   και φυλάγεται εδώ, μετά ο ήρωας ξαναχρησιμοποιεί το spr_buf.
;---------------------------------------------------------------------
spr_save_para:  ld   hl,spr_buf
                ld   de,para_buf
                ld   bc,SPR_BUFSZ
                ldir
                ret
