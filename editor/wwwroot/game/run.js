...
(function (D, G, R) {
  ...
  function frame() {
    ...
    // ΠΟΥ ΒΓΑΖΕΙ Η ΤΗΛΕΜΕΤΑΦΟΡΑ ΠΟΥ ΠΑΤΑΣ. Ο προορισμός δηλώνεται και μπορεί
    // να είναι οπουδήποτε στην αίθουσα: χωρίς σημάδι ο παίκτης πατάει και
    // μαθαίνει πού βγήκε αφού βγει. Το βελάκι μπαίνει σε ΚΕΝΟ κελί ΔΙΠΛΑ στον
    // προορισμό και τον δείχνει: πάνω του θα έκρυβε ακριβώς αυτό που δείχνει.
    const tph = room.teleportHint(hero.bodyCell());
    if (tph) screen.tpArrow(tph[0], tph[1], tph[2]);
  ...
  // ΓΙΑ ΤΑ ΤΕΣΤ, ΟΧΙ ΓΙΑ ΤΟ ΠΑΙΧΝΙΔΙ: το μήνυμα και η ενέργεια πρέπει να
