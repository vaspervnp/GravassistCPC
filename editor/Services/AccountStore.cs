...
public sealed class AccountStore
{
    ...
    /// <summary>
    /// Βγάζει τον λογαριασμό από τη λίστα.
    ///
    /// ΔΙΑΦΕΡΕΙ ΑΠΟ ΤΗΝ ΑΝΑΚΛΗΣΗ: η ανάκληση τον κρατά καταγεγραμμένο ως
    /// «όχι», οπότε αν ξαναζητήσει πρόσβαση παραμένει κομμένος — είναι το
    /// μπλόκο. Η διαγραφή τον ΞΕΧΝΑ: αν ξαναμπεί, εμφανίζεται πάλι σαν
    /// καινούριο αίτημα. Για καθάρισμα της λίστας, όχι για αποκλεισμό.
    ///
    /// Ο ΦΑΚΕΛΟΣ ΤΟΥ ΔΕΝ ΑΓΓΙΖΕΤΑΙ. Ένα κλικ σε οθόνη διαχείρισης δεν πρέπει
    /// να καταστρέφει τη δουλειά κανενός· τα αρχεία σβήνονται με το χέρι, αν
    /// και όταν το αποφασίσεις.
    /// </summary>
