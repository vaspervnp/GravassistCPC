...
(Τα +5/+9/+17 ισχύουν για SET_ROOMS = 4. Το HEADER τα υπολογίζει, και το
...
# (ήχος, οθόνες τέλους, νέοι τύποι κελιών) και το σετ έφτασε στο 93% του. Με
# 4 αίθουσες ανά αρχείο, το κάθε ROOMSnn.BIN χωράει άνετα και τα σετ γίνονται
# περισσότερα — ο φορτωτής υποστηρίζει μέχρι 99, δηλαδή 396 αίθουσες.
#
# Το τίμημα είναι μία φόρτωση από δισκέτα κάθε τέσσερις αίθουσες αντί για
# κάθε σαράντα. Σε πόρτα που ήδη ξαναζωγραφίζει όλη την οθόνη, δεν φαίνεται.
SET_ROOMS = 4
...
#
# ΣΤΑΘΕΡΕΣ ΘΕΣΕΙΣ ΤΟΥ 1 KB, όχι πίνακας offsets: ο Z80 βρίσκει το σετ με μια
# ολίσθηση αντί για αναζήτηση, και δεν χρειάζεται δεύτερος πίνακας που θα
# μπορούσε να ξεσυγχρονιστεί. Το τίμημα είναι το κενό στο τέλος κάθε θέσης
# (σήμερα 164 και 536 bytes) και ΕΝΑ ΣΚΛΗΡΟ ΟΡΙΟ: κανένα σετ δεν επιτρέπεται
# να ξεπεράσει το SLOT_SIZE — αυστηρότερο από τον buffer του CPC.
BANK_SIZE = 0x4000
FIRST_BANK = 4                  # μπλοκ 4..7 = οργανώσεις #C4..#C7
BANK_COUNT = 4
SLOT_SIZE = 1024
SLOTS_PER_BANK = BANK_SIZE // SLOT_SIZE         # 16
MAX_SETS = SLOTS_PER_BANK * BANK_COUNT          # 64 σετ = 256 αίθουσες
...
def set_count():
    ...
    """
    return max((i for i, _, _ in all_sets()), default=0)
