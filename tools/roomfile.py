...
ΕΓΓΡΑΦΗ ΑΙΘΟΥΣΑΣ:
    ...
    (col,row,cool,auto)*  #FF      χρόνοι πυργίσκων, σε δευτερόλεπτα
    (col,row,w,h,dc,dr,ch,sp)* #FF κινούμενες πλατφόρμες
                                   (bit 7 του ch: ξεκινά σταματημένη)
    RLE κελιά, μέχρι να βγουν COLS*ROWS:
        0ttttttt                   ΕΝΑ κελί τύπου t
        1ttttttt count             count κελιά τύπου t

Όλοι οι πίνακες τερματίζονται με #FF, ώστε οι βρόχοι σάρωσης του src/hero.asm
να δουλεύουν πάνω στο αρχείο χωρίς αντιγραφή.
"""
...
# 3 = πέμπτος πίνακας: οι δύο χρόνοι κάθε πυργίσκου.
# 4 = έκτος πίνακας: οι κινούμενες πλατφόρμες.
...
# CPC μίκραινε καθώς μεγάλωνε ο κώδικας. Τώρα ξαναγύρισε στο 4 και ΜΕΝΕΙ εκεί:
# ο buffer δεν είναι πια κύρια μνήμη αλλά τα δεύτερα 2 KB της οθόνης, οπότε
# δεν συρρικνώνεται με κάθε γραμμή που προσθέτουμε.
#
# Το τίμημα είναι μία φόρτωση από δισκέτα ανά τέσσερις αίθουσες — και ΜΟΝΟ σε
# μηχάνημα 64K: με τις τράπεζες όλα τα σετ είναι ήδη στη μνήμη και η πόρτα δεν
# αγγίζει τον δίσκο καθόλου (src/bank.asm).
SET_ROOMS = 4
HEADER = 3 + 1 + 1 + SET_ROOMS + 2 * SET_ROOMS          # = 17 με SET_ROOMS 4
...
BANK_COUNT = 4
SLOT_SIZE = 2048
SLOTS_PER_BANK = BANK_SIZE // SLOT_SIZE         # 8
...
def room_record(room):
    ...
    # ΕΚΤΟΣ ΠΙΝΑΚΑΣ: οι κινούμενες πλατφόρμες, εξάδες.
    #
    # ΣΕ ΚΕΛΙΑ, ΚΑΙ ΜΕ ΤΟ ΜΕΓΕΘΟΣ ΜΕΣΑ.
    #
    # Το μέγεθος ΔΕΝ μπορεί να βγει από το πλέγμα εδώ: το Room σβήνει τα κελιά
    # «M» στη φόρτωση — δείχνουν πού ξεκινάει, δεν είναι υλικό — και το RLE από
    # κάτω γράφεται ΜΕΤΑ από αυτό. Ο Z80 θα έβλεπε άδειο δωμάτιο και πλατφόρμα
    # χωρίς διαστάσεις. Ταξιδεύει λοιπόν ό,τι δεν φαίνεται πια στο πλέγμα.
    for pl in room.platforms:
        c0, r0 = pl["ax"] // P.CELL, (pl["ay"] - P.GRID_Y0) // P.CELL
        dc, dr = pl["bx"] // P.CELL, (pl["by"] - P.GRID_Y0) // P.CELL
        # ΤΟ BIT 7 ΤΟΥ ΚΑΝΑΛΙΟΥ ΛΕΕΙ «ΞΕΚΙΝΑΕΙ ΣΤΑΜΑΤΗΜΕΝΗ». Το κανάλι είναι
        # 0..7, οπότε το ψηλό bit είναι ελεύθερο — και χρειάζεται, γιατί ο Z80
        # δεν έχει από πού αλλού να το μάθει: ο τύπος του κελιού («M» ή «m»)
        # έχει ήδη σβηστεί από το πλέγμα όταν γράφεται το RLE από κάτω.
        flag = 0 if pl["moving"] else 0x80
        out += bytes((c0, r0, pl["w"] // P.CELL, pl["h"] // P.CELL,
                      dc, dr, pl["chan"] | flag, pl["speed"] & 0xFF))
    out.append(0xFF)

    flat = [v for row in room.cells for v in row]

    # Ο ΕΠΙΒΑΤΗΣ ΞΑΝΑΜΠΑΙΝΕΙ ΣΤΟ ΠΛΕΓΜΑ, ΓΙΑ ΝΑ ΤΟΝ ΒΡΕΙ Ο Z80 ΚΑΙ ΝΑ ΤΟΝ ΒΓΑΛΕΙ
    # ΠΑΛΙ. Ο διακόπτης που ταξιδεύει πάνω στην πλατφόρμα φεύγει από τα κελιά
    # στη φόρτωση, όπως και η ίδια: κινείται ανά pixel, δεν είναι πια κελί. Αν
    # όμως δεν γραφτεί εδώ, ο Z80 σαρώνει το κελί από πάνω της, βρίσκει μηδέν
    # και η πλατφόρμα φτάνει στη δισκέτα ΧΩΡΙΣ διακόπτη — δούλευε παντού εκτός
    # από το σίδερο. Το plat_load τον ξαναβγάζει μόλις τον διαβάσει, οπότε το
    # πλέγμα του παιχνιδιού μένει ίδιο· έτσι ταξιδεύει και το κανάλι του, από
    # τον πίνακα ιδιοτήτων, χωρίς δικά του bytes.
    for pl in room.platforms:
        if not pl["rider"]:
            continue
        cc = (pl["x"] + pl["rdx"]) // P.CELL
        cr = (pl["y"] - P.CELL - P.GRID_Y0) // P.CELL
        flat[cr * P.COLS + cc] = pl["rider"]
...
def parse_set(data, name="(σετ)"):
    ...
    for i in range(count):
        ...
        room["platforms"], pos = table(pos, 8, "πλατφορμών")
        try:
