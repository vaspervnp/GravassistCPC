...
def _frame(kind, active):
    ...
    elif kind == "PLATFORM":                 # πλάκα με πλαίσιο· active = ακίνητη
        # ΓΕΜΑΤΟ ΚΕΛΙ ΜΕ ΠΛΑΙΣΙΟ ΟΛΟΓΥΡΑ, γιατί είναι στερεή από ΠΑΝΤΟΥ. Είχε
        # φωτεινό χείλος μόνο πάνω, όσο ήταν μονόδρομη: εκείνο έλεγε «εδώ είναι
        # το πάτωμα» και τώρα θα ήταν ψέμα — πατάς και ανάποδα και πλάγια.
        # Τα δόντια μένουν: λένε ότι είναι μηχανισμός και όχι τοίχος.
        rect(f, 0, 0, 7, 7, hi)
        rect(f, 1, 1, 6, 6, body, fill=True)
        if active:
