...
for _t in (SPIKE_U_OFF, SPIKE_L_OFF, SPIKE_D_OFF, SPIKE_R_OFF):
    PROPS[_t] |= F_SOLID
# Ο πυργίσκος είναι στερεός: εμπόδιο που και σε σταματά και σε πυροβολεί. Δεν
# είναι F_DEADLY — δεν πονάει με την αφή, μόνο το βέλος του πονάει.
for _t in TURRETS:
    PROPS[_t] |= F_SOLID
...
class Hero:
    ...
    def update(self, walk=0, run=False):
        ...
        self.clock += (CPC_VSYNC_RUN if (walk and run)
