...
# ΤΟ ΜΗΝΥΜΑ ΕΙΣΟΔΟΥ ΤΗΣ ΑΙΘΟΥΣΑΣ. Μία γραμμή, γιατί το MODE 1 έχει 40 στήλες
# κειμένου και ένα κεντραρισμένο μήνυμα που τυλίγεται παύει να είναι
# κεντραρισμένο. Το όριο αφήνει ένα κενό κελί δεξιά κι αριστερά.
MSG_MAX = 38
...
class Room:
    def __init__(self, text):
        ...
        self.all_chan = 0
        # Μήνυμα που δείχνεται ΠΡΙΝ ζωγραφιστεί η αίθουσα, κεντραρισμένο.
        self.message = ""
        ...
        for ln in text.splitlines():
            ...
            # ΜΗΝΥΜΑ ΕΙΣΟΔΟΥ: μία γραμμή κειμένου, κεντραρισμένη, πριν
            # ζωγραφιστεί η αίθουσα. Ό,τι ακολουθεί το «msg » είναι το κείμενο
            # αυτούσιο — ΔΕΝ κόβεται σε λέξεις, γιατί τα κενά μετράνε στο
            # κεντράρισμα και ο σχεδιαστής τα βλέπει όπως τα γράφει.
            m = re.match(r"\s*msg\s+(.*?)\s*$", ln, re.I)
            if m:
                self.message = m.group(1)[:MSG_MAX]
    ...
    def teleport_hint(self, cell):
        """Πού βγάζει η τηλεμεταφορά αυτού του κελιού, και πού μπαίνει το σημάδι.

        OUT: (φορά, στήλη, γραμμή) — ένα ΚΕΝΟ κελί ΔΙΠΛΑ στον προορισμό, και η
        φορά από εκείνο προς τον προορισμό, ώστε το βελάκι να τον δείχνει.

        ΓΙΑΤΙ ΔΙΠΛΑ ΚΑΙ ΟΧΙ ΠΑΝΩ: πάνω του σκεπάζει το πλακίδιο, δηλαδή κρύβει
        ακριβώς το πράγμα που δείχνει. ΓΙΑΤΙ ΚΕΝΟ: αλλιώς σκεπάζει ό,τι άλλο
        τύχει να είναι εκεί, και ο παίκτης χάνει πληροφορία αντί να κερδίζει.

        Η αναζήτηση ξεκινά από τη μεριά ΑΠ' ΟΠΟΥ ΕΡΧΕΣΑΙ και γυρίζει τον κύκλο:
        εκεί το βελάκι πέφτει ανάμεσα στις δύο πλατφόρμες και διαβάζεται σαν
        διαδρομή. Αν δεν υπάρχει κανένα κενό διπλανό, μπαίνει πάνω του — ένα
        σημάδι που κρύβει κάτι είναι καλύτερο από κανένα σημάδι.
        """
        ...
        for i in range(8):
            k = (g + 4 + i) % 8         # πρώτα η μεριά που έρχεσαι
            dx, dy = GSTEP[k]
            nc, nr = dest[0] + dx, dest[1] + dy
            if not (0 <= nc < COLS and 0 <= nr < ROWS):
                continue
            if self.cell(nc, nr) == EMPTY:
                # Η φορά ΑΠΟ το κελί ΠΡΟΣ τον προορισμό: το αντίθετο του
                # βήματος που μας έφερε εδώ.
                return ((k + 4) % 8, nc, nr)
    ...
    def solid_at(self, px, py):
        ...
        #
        # ΣΤΕΡΕΗ ΑΠΟ ΠΑΝΤΟΥ, ΣΑΝ ΤΟΙΧΟΣ. Ήταν μονόδρομη — στερεή μόνο από πάνω —
        # και αυτό την έκανε ανελκυστήρα ενός άξονα: με βαρύτητα προς τα πάνω ο
        # ήρωας την περνούσε σαν να μην υπήρχε, ενώ ο τοίχος δίπλα του γινόταν
        # πάτωμα. Τώρα ό,τι ισχύει για τον τοίχο ισχύει και για εκείνην: πατάς
        # πάνω της με όποια φορά κι αν έχεις, και σε κουβαλάει.
...
class Hero:
    ...
    def plat_riding(self, p):
        ...
        """
        # ΜΕ ΟΠΟΙΑ ΦΟΡΑ ΚΙ ΑΝ ΕΧΕΙ: το off(g, a, k) μετράει προς ΤΑ ΔΙΚΑ ΤΟΥ
        # πόδια, οπότε τα ίδια εννέα σημεία δουλεύουν και ανάποδα και πλάγια.
