...
class Room:
    ...
    def teleport_hint(self, cell):
        """Πού βγάζει η τηλεμεταφορά αυτού του κελιού.

        OUT: (φορά, στήλη, γραμμή) — ΤΟ ΚΕΛΙ ΠΡΟΟΡΙΣΜΟΥ και η φορά της
        διαδρομής προς τα εκεί. Το βελάκι μπαίνει ΠΑΝΩ στον προορισμό: αυτό
        είναι το ερώτημα του παίκτη («πού θα βρεθώ;»), όχι «προς τα πού είναι».
        Δίπλα του δεν λέει τίποτα που να μη λέει ήδη το σχήμα του βέλους.
        None αν η τηλεμεταφορά δεν είναι δηλωμένη.
        """
