...
SPIKE_TICKS = 10
# Μετά από πέρασμα πόρτας, η ΙΔΙΑ (ή οποιαδήποτε) πόρτα αγνοείται για ένα
# δευτερόλεπτο. Το σημείο άφιξης είναι αναγκαστικά κοντά στην πόρτα
# επιστροφής, οπότε ένα γλίστρημα λίγων pixel σε αφήνει να ξαναμπείς και να
# πηγαινοέρχεσαι. Χωρίς αυτό, κάθε άφιξη είναι μια λεπτή ισορροπία που ο
# σχεδιαστής πρέπει να πετύχει· με αυτό, απλώς προλαβαίνεις να φύγεις.
EXIT_COOL = 50          # frames στα 50 Hz = 1 δευτερόλεπτο
...
class Hero:
    ...
    def touch_objects(self):
        ...
        elif t == EXIT and self.exit_cool == 0:
            self.won = True
    ...
    def enter_room(self):
        """Ο ήρωας μόλις μπήκε από πόρτα: κλείδωσε τις πόρτες για λίγο."""
        self.exit_cool = EXIT_COOL
    ...
    def update(self, walk=0, run=False):
        ...
        """
        self.moved_cells = []
        # ΠΡΙΝ το touch_objects: το πρώτο frame μετά την άφιξη πρέπει να μετράει
        # στο κλείδωμα, αλλιώς η πόρτα ελέγχεται μία φορά με cooldown 0.
        if self.exit_cool:
            self.exit_cool -= 1
        self.crate_step()
