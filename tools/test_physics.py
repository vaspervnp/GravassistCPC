...
def main():
    ...
    # ΚΑΙ ΟΙ ΤΕΣΣΕΡΙΣ ΦΟΡΕΣ. Η ζώνη δεν «παγώνει ό,τι είχες»: επιβάλλει τη ΔΙΚΗ
    # της, και το πλακίδιο τη δείχνει. Ο ήρωας μπαίνει με κάθε μία από τις οκτώ
    # φορές και πρέπει να βγαίνει πάντα με εκείνη της ζώνης.
    for ch, want in ((":", 0), ("8", 4), ("4", 2), ("6", 6)):
        rows3 = [list("#" * 40)] + [list("#" + "." * 38 + "#") for _ in range(22)] \
            + [list("#" * 40)]
        for r in range(10, 20):
            for c in range(10, 20):
                rows3[r][c] = ch
        rm3 = P.Room(";\n" + "\n".join("".join(r) for r in rows3) + "\ngravity 0")
        got = set()
        for g0 in range(8):
            h3 = P.Hero(rm3, 15 * 8 + 4, P.GRID_Y0 + 15 * 8 + 4, g0)
            h3.update(0)
            got.add(h3.g)
        check(f"ζώνη «{ch}»: επιβάλλει τη φορά {want} από όποια κι αν μπεις",
              got == {want}, str(sorted(got)))

    # …και η ΙΔΙΑ φορά που λέει το πλακίδιο, με τον κανόνα των αγκαθιών.
    check("το FACING των τεσσάρων ζωνών δίνει τις τέσσερις ορθές φορές",
          sorted((P.FACING[t] + 4) % 8 for t in P.GRAVLOCKS) == [0, 2, 4, 6],
          str([(P.TYPE_NAMES[t], (P.FACING[t] + 4) % 8) for t in P.GRAVLOCKS]))
    ...
    # ΣΤΕΡΕΗ ΑΠΟ ΠΑΝΤΟΥ, ΣΑΝ ΤΟΙΧΟΣ. Ήταν μονόδρομη — μόνο από πάνω — και αυτό
    # την έκανε ανελκυστήρα ενός άξονα: με βαρύτητα πάνω ο ήρωας την περνούσε
    # σαν να μην υπήρχε, ενώ ο τοίχος δίπλα του γινόταν πάτωμα.
    rm = proom("24 14")
    pl = rm.platforms[0]
    mid = (pl["x"] + 4, pl["y"] + 4)
    for g in (0, 4, 2, 6):
        rm.probe_g = g
        check(f"βαρύτητα {g}: στερεή", rm.solid_at(*mid))
    rm.probe_g = 0
    check("…και δίπλα της, κενό", not rm.solid_at(pl["x"] - 4, pl["y"] + 4))

    # ΚΑΙ ΣΤΗΝ ΠΡΑΞΗ, ΚΑΙ ΣΤΙΣ ΤΕΣΣΕΡΙΣ ΦΟΡΕΣ: ο ήρωας προσγειώνεται πάνω της
    # και ΤΑΞΙΔΕΥΕΙ μαζί της. Χωρίς το ταξίδι θα ήταν σκέτος τοίχος που
    # ξεγλιστράει από κάτω σου — και το ένα δεν συνεπάγεται το άλλο.
    for g, dx, dy in ((0, 0, -1), (4, 0, +1), (2, +1, 0), (6, -1, 0)):
        rm = proom("24 14", cells=3)
        pl = rm.platforms[0]
        # Ο ήρωας στην πλευρά όπου «πέφτει πάνω της» με αυτή τη βαρύτητα.
        h = P.Hero(rm, pl["x"] + pl["w"] // 2 + dx * (pl["w"] // 2 + 8),
                   pl["y"] + pl["h"] // 2 + dy * (pl["h"] // 2 + 8), g)
        for _ in range(30):
            h.update(0)
        landed = h.on_platform() if hasattr(h, "on_platform") else \
            any(rm.plat_at(*P.off_point(h, a, k))
                for a in (-P.FOOT_A, 0, P.FOOT_A)
                for k in range(P.FEET_B, P.FEET_B + 3)) \
            if hasattr(P, "off_point") else h.plat_riding(pl)
        check(f"βαρύτητα {g}: στέκεται πάνω της", landed,
              f'ήρωας ({h.x},{h.y}) πλατφόρμα ({pl["x"]},{pl["y"]})')
        x0, y0 = h.x, h.y
        px0, py0 = pl["x"], pl["y"]
        for _ in range(40):
            h.update(0)
        check(f"βαρύτητα {g}: ταξιδεύει μαζί της",
              (h.x - x0, h.y - y0) == (pl["x"] - px0, pl["y"] - py0)
              and (pl["x"], pl["y"]) != (px0, py0),
    ...
    # ΤΟ ΒΕΛΑΚΙ ΤΗΣ ΤΗΛΕΜΕΤΑΦΟΡΑΣ: πού βγάζει αυτή που πατάς.
    for dxdy, want in (((5, 0), 6), ((0, -7), 4), ((-4, -4), 3), ((0, 0), None),
                       ((20, 1), 6), ((1, 20), 0), ((3, -2), 5),
                       # 29 μοίρες κάτω από την οριζόντια: πιο κοντά στη
                       # διαγώνιο (45) παρά στην οριζόντια — η αναμονή μου ήταν λάθος
                       ((-9, 5), 1)):
        check(f"οκτάντας {dxdy}", P.Room.octant_of(*dxdy) == want,
              str(P.Room.octant_of(*dxdy)))

    trows = [list("#" * 40)] + [list("#" + "." * 38 + "#") for _ in range(22)] \
        + [list("#" * 40)]
    trows[22][10] = "T"
    trows[5][30] = "T"
    trows[12][0] = "T"                      # στην ΑΚΡΗ: το βελάκι μαζεύεται
    trm = P.Room(";\n" + "\n".join("".join(r) for r in trows)
                 + "\ngravity 0\ntp 10 22 30 5\ntp 30 5 10 22\ntp 0 12 30 5")
    hint = trm.teleport_hint((10, 22))
    check("δείχνει προς το ταίρι της", hint[0] == P.Room.octant_of(20, -17),
          str(hint))
    check("…και μπαίνει στο ΔΙΠΛΑΝΟ κελί, όχι στο δικό της",
          (hint[1], hint[2]) == (11, 21), str(hint))
    # ΣΤΗΝ ΑΚΡΗ ΤΟΥ ΠΛΕΓΜΑΤΟΣ ΔΕΝ ΒΓΑΙΝΕΙ ΕΞΩ: το ταίρι είναι πάνω δεξιά, το
    # διπλανό κελί θα ήταν η στήλη -1. Μισοκρυμμένο είναι καλύτερο από άφαντο.
    edge = trm.teleport_hint((0, 12))
    check("στην άκρη μαζεύεται πάνω στο κελί της",
