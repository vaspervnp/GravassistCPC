...
# --- ΔΕΞΙΑ ΑΚΡΗ: το κέντρο του κελιού δεν χωράει σε byte ---------------
#
# Η γραμμή είναι 320 pixel. Το κέντρο υπολογιζόταν με τρία `add a,a` σε ΟΚΤΩ
# bit, οπότε από τη στήλη 32 και πέρα το col*8 ξεπερνούσε το 255 και έχανε το
# ψηλό bit: ο πυργίσκος της στήλης 36 γεννούσε βέλη στο x=41 αντί για 297 —
# στην ΑΡΙΣΤΕΡΗ άκρη της οθόνης. Το μοντέλο και ο browser δεν το είχαν ποτέ,
# γιατί εκεί οι ακέραιοι δεν έχουν πλάτος· κανένα τεστ δεν είχε πυργίσκο πέρα
# από τη στήλη 31.
print("--- πυργίσκος στη δεξιά άκρη, στον Z80")
for col in (10, 31, 32, 36, 38):
    t = z80_room([(col, 12, "I")], targ=[(col, 12, 1, 0)])
    t.call("TURRET_LOAD")
    t.poke16(t.sym("HERO_X"), col * P.CELL + P.CELL // 2)
    t.poke(t.sym("HERO_Y"), bytes([P.GRID_Y0 + 16 * P.CELL]))
    t.call("TURRET_STEP")
    on = t.peek(t.sym("ARROW_TAB"))[0]
    x = t.peek16(t.sym("ARROW_TAB") + 1)
    check(f"στήλη {col}: το βέλος γεννιέται πάνω στον πυργίσκο",
          on == 1 and x == col * P.CELL + P.CELL // 2,
          f"x={x}, περίμενα {col * P.CELL + P.CELL // 2}")
